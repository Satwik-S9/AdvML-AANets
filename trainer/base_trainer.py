...
from parent.utils.incremental.compute_features import compute_features
...
class BaseTrainer(object):
    ...
    def set_dataset_variables(self):
        ...
        valdir = os.path.join(self.args.data_dir, 'valid')

        # Set Dataloaders --Transforms
    ...
    def init_fusion_vars(self):
        ...
        # Birds Dataset, the number of blocks: 4
        if self.args.branch_mode == 'dual':
    ...
    def init_class_order(self):
        ...
        if osp.exists(order_name):
            ...
            order = parent.utils.misc.unpickle(order_name)
        else:
            ...
            parent.utils.misc.savepickle(order, order_name)
    ...
    def init_prototypes(self, dictionary_size, order, X_train_total, Y_train_total):
        ...
        # Birds Dataset, save the paths for the training samples if an array
